...
### T143 — Dockerización (despliegue propio y distribución)
**Estado:** DONE
**Dep:** ninguna (empaqueta lo ya construido, no añade funcionalidad de dominio)  
Petición directa del usuario (2026-09-24): "para realizar un correcto despliegue y ejecución, tanto propio como ajeno si distribuyo la herramienta, dockerízalo".

**Nota:** Sin ambigüedad de producto (a diferencia de T135/136/137, no hubo `AskUserQuestion`) pero con decisiones de arquitectura reales por resolver, todas con un problema conocido y una solución estándar -- se procedió documentando el razonamiento en vez de preguntar, mismo criterio que T138:

1. **Credenciales sin keyring de SO real.** `CredentialStore` (docs/AGENTS.md #21) delega en la librería `keyring`, que auto-detecta Credential Manager/Keychain en Windows/Mac -- un contenedor Linux mínimo no tiene ningún backend real disponible (`NoKeyringError` en el primer `set_password`). Resuelto con `keyrings.cryptfile` (nueva dependencia) + `app/config/keyring_setup.py` (nuevo): `configure_container_keyring()` activa `CryptFileKeyring` con la contraseña de `KEYRING_CRYPTFILE_PASSWORD` SOLO si esa variable existe -- en un run nativo (sin la variable) es no-op total, el keyring real de SO se usa exactamente igual que antes. Sin contraseña por defecto/hardcodeada (una imagen distribuida con una contraseña compartida predecible sería una vulnerabilidad real) -- el usuario la genera (`openssl rand -base64 32`).
2. **Exposición de red.** docs/AGENTS.md #19 ("bind to loopback by default") se traduce, en el mundo Docker, a publicar puertos como `127.0.0.1:PUERTO:PUERTO` en vez de `0.0.0.0` -- ambos servicios de `docker-compose.yml` lo hacen por defecto.
3. **CORS.** En vez de dejar el origen de Vite hardcodeado (`app/main.py`), `Settings` (`app/config/settings.py`) gana `cors_origins`/`cors_origins_list` (string separado por comas, no JSON -- se queda como un valor de una línea sin comillas en un `.env`) leído vía `LEARNINGOS_CORS_ORIGINS`, con el mismo default de siempre. En la práctica el compose ni lo necesita: `frontend/nginx.conf` hace reverse proxy de `/api/` al backend, mismo origen, cero CORS.
4. **Ollama.** No viene incluido por defecto (imagen pesada, complejidad de GPU) -- servicio opcional vía `docker compose --profile ollama up`, documentado con el gotcha real de red interna (`http://ollama:11434` desde dentro del compose, `http://host.docker.internal:11434` para un Ollama del host -- `localhost` dentro del contenedor del backend NO es ninguno de los dos).

Backend (`backend/Dockerfile`, multi-stage con la imagen oficial `ghcr.io/astral-sh/uv`): dos capas `uv sync` separadas (deps primero, código después) para cachear la capa lenta; usuario no-root; `HOME=/data` apunta al mismo volumen persistente que la SQLite DB, así el archivo cifrado de `keyrings.cryptfile` sobrevive igual que la DB; `CMD` corre `alembic upgrade head` (idempotente, docs/AGENTS.md #8) antes de `uvicorn --host 0.0.0.0` en cada arranque, no solo el primero. Frontend (`frontend/Dockerfile`): build de Vite con `VITE_API_BASE_URL=/api/v1` fijado en build-time, servido por `nginx:alpine` con proxy a `/api/`. `docker-compose.yml`: dos servicios + volumen nombrado para la DB, bind mount de `VAULT_HOST_PATH` (variable requerida, sin default) a `/vault`, `KEYRING_CRYPTFILE_PASSWORD` requerida también (ambas usan la sintaxis `${VAR:?mensaje}` de Compose para fallar rápido y claro si faltan) y `HEALTHCHECK` en el backend que gatea el arranque del frontend (`condition: service_healthy`).

Verificación real de punta a punta, no solo revisión estática -- Docker Desktop estaba disponible en esta máquina, se arrancó, se construyeron ambas imágenes y se levantó el stack completo: `docker compose up -d` → backend queda `Healthy` antes de que el frontend arranque (confirma el `HEALTHCHECK`) → `GET /api/v1/health` responde tanto directo (`:8000`) como a través del proxy de nginx (`:8080/api/...`) → vault bind-mount real: un `note.md` escrito desde el host aparece correctamente al escanear `/vault` desde dentro del contenedor → credencial real: `CredentialStore().set/get()` dentro del contenedor, ejecutado tras importar `app.main` (dispara `configure_container_keyring()` igual que el arranque real de uvicorn), guarda y recupera un secreto correctamente vía el backend cifrado -- confirmado el archivo real en `/data/.local/share/python_keyring/cryptfile_pass.cfg` -- y el secreto sobrevive tanto un `docker compose restart backend` como un `docker compose down` + `up` completo (recreación total del contenedor), igual que el estado de onboarding en SQLite. Stack completo derribado (`docker compose down -v`) y artefactos de prueba (`.env` de prueba, vault temporal) borrados al terminar, sin dejar residuos.

15 tests nuevos backend (4 en `test_settings.py` -- parseo de `cors_origins_list` --, 2 en `test_keyring_setup.py` -- no-op sin variable de entorno, activación real del backend cryptfile con contraseña, restaurando el keyring global en el `finally` para no contaminar otros tests) + 998/998 suite backend completa (998 = 992 previos + 6 nuevos -- la diferencia de conteo son los 4+2 de este párrafo), ruff/mypy limpios. Sin tests de frontend nuevos (ningún archivo de `frontend/src/` cambia) pero `npm run build` (`tsc -b && vite build`) se ejecutó realmente dentro del build de Docker y tuvo éxito. `.gitattributes` nuevo (fuerza LF en `Dockerfile`/`docker-compose.yml`/`nginx.conf`/`*.sh` -- este repo se desarrolla en Windows, CRLF en un script con shebang rompería el contenedor Linux; se evitó además tener ningún script con shebang independiente, todo el arranque vive inline en el `CMD` del Dockerfile, precisamente para no depender de esa normalización). `DEVELOPMENT.md`/`README.md` actualizados con la sección Docker y troubleshooting; de paso, corregida la versión de Node.js desactualizada en `DEVELOPMENT.md` (decía "20+", `package.json` `engines` exige `^22.22.2 || ^24.15.0 || >=26.0.0`, CI fija `22.22.2`) y el "Status" desactualizado de `README.md` (123/138, Fase 12 "actual" -- ya no era cierto desde hace varias tareas de esta sesión).

`uv` en sí no estaba instalado/en PATH en esta máquina (todo el trabajo previo de la sesión usó el `.venv` preexistente directamente vía `python -m pytest`/`ruff`/`mypy`) -- instalado vía `pip install uv` dentro de ese mismo `.venv` específicamente para regenerar `backend/uv.lock` con la nueva dependencia `keyrings.cryptfile` (`uv lock`), verificado con `uv sync --frozen` real (no solo el `uv sync` sin `--frozen` que ya usa el Dockerfile) y la suite completa (`pytest`/`ruff`/`mypy`) otra vez en verde después.

### T144 — Incluir el cuerpo de la respuesta del proveedor en `AIProviderUnavailableError`
**Estado:** DONE
**Dep:** ninguna (arregla un comportamiento ya existente en los 7 adapters)  
Encontrada depurando en vivo con el usuario, desplegado ya en Docker (T143): al generar un roadmap contra Anthropic real, `POST /v1/messages` devolvía 400 y el único mensaje visible era `Client error '400 Bad Request' for url '...'` -- el texto genérico de `httpx.HTTPStatusError`, sin el cuerpo de la respuesta, que es exactamente donde Anthropic (y cualquier proveedor OpenAI-compatible) explica QUÉ está mal (modelo inválido, campo mal formado, límite de cuota). Ningún adapter lo exponía nunca -- los 7 (`anthropic.py`, `ollama.py`, `ollama_embeddings.py`, `_openai_compatible_base.py` -- cubre `openai`/`openrouter`/`nvidia_nim`/`openai_compatible` por herencia --, `_openai_compatible_embeddings_base.py`) compartían el mismo `except httpx.HTTPError as exc: raise AIProviderUnavailableError(str(exc))`.

**Nota:** Único punto de integración -- `app/ai/adapters/_http_errors.py` (nuevo), `provider_unavailable_error(exc)` -- distingue `httpx.HTTPStatusError` (sí llegó respuesta, incluye `exc.response.text`) de cualquier otro `httpx.HTTPError` (`ConnectError`/`TimeoutException`, nunca hubo respuesta que leer, mismo mensaje de siempre). Los 5 archivos con el patrón duplicado pasan a `raise provider_unavailable_error(exc) from exc`; los otros 3 adapters (`openai.py`/`openrouter.py`/`nvidia_nim.py`) ya heredan de `OpenAICompatibleAdapter` sin override de `generate()`, así que quedan cubiertos sin tocarlos. Cuerpo de respuesta considerado seguro de mostrar -- describe un error de la propia API sobre NUESTRA petición, nunca contiene la API key ni contenido del vault (docs/AGENTS.md #21).

De paso, al investigar esto se confirmó un hallazgo más grande y separado: `check_provider_capability()` (`app/ai/capability_check.py`) nunca ha hecho una llamada de red real pese a que su propio docstring decía que se añadiría "once each provider's real adapter lands (T051-T056)" -- los 7 adapters existen desde hace muchas tareas y la función sigue sin usarlos, solo valida que los campos no estén vacíos. O sea: "Test connection" en el onboarding puede reportar éxito con un modelo o API key inválidos, y el usuario no se entera hasta la primera llamada real (como pasó aquí). Viola `docs/AI_CONTRACTS.md` #19 ("Before activation, validate connection, credential if required, model and minimum structured-output capability") tal cual está escrito. No se arregla en esta tarea -- es un cambio más grande (llamada real async desde dos rutas HTTP distintas, decisión de qué request mínima mandar por proveedor) -- señalado al usuario como tarea aparte.

9 tests nuevos (8 en `test_http_errors.py` -- cuerpo incluido en status error, fallback correcto para error de conexión, parametrizado sobre 401/403/404/429/500/503 --, 1 en `test_anthropic.py` -- integración real a través del adapter completo, no solo el helper aislado) + 1007/1007 suite backend completa (1007 = 998 previos + 9 nuevos), ruff/mypy limpios. Sin cambios de frontend.

### T145 — `check_provider_capability` valida la conexión de verdad
**Estado:** DONE
**Dep:** T144 (error con cuerpo de respuesta), T051-T056 (adapters reales)  
Cierra el hallazgo señalado al terminar T144: `check_provider_capability` (T018) nunca hizo una llamada de red real pese a que su propio docstring lo anunciaba "once each provider's real adapter lands (T051-T056)". Ese "until then" llevaba muchas tareas siendo falso -- viola `docs/AI_CONTRACTS.md` #19 tal cual está escrito ("validate connection, credential if required, model and minimum structured-output capability").

**Nota:** No se preguntó al usuario pese a ser un cambio de comportamiento en una ruta de seguridad/fiabilidad real -- las dos ambigüedades reales tenían respuesta ya precedentada en el propio código, no una decisión de producto nueva: (1) qué request mínima mandar -- se reutiliza exactamente la misma forma trivial (`{"answer": "..."}`, un solo campo string) que `backend/tests/live/test_provider_smoke.py` ya usa para sus propios checks en vivo, no una decisión inventada aquí; (2) si bloquear la UI mientras se valida -- ya bloqueaba (el usuario ya espera la respuesta del POST de "Test connection"), simplemente antes esa espera no hacía nada útil.

Diseño en dos etapas, no una reescritura de `check_provider_capability`: se mantiene intacta (estructural, sin red, sigue siendo el primer filtro barato -- "sin credencial" no necesita gastar una petición real para fallar). Nueva `validate_provider_connection` (async) es la etapa en vivo, llamada SOLO si la estructural ya dio `ok=True`. `mock` queda exento explícitamente -- determinista y offline por diseño (`app/ai/adapters/mock.py`), "alcanzable" no es una pregunta con sentido para él, y no tiene respuesta configurada para el schema de esta prueba fuera de tests, así que llamarlo aquí rompería uno de los caminos explícitamente cero-setup de la app (`docs/DEVELOPMENT.md`: "the bundled Mock provider ... is enough to explore the app"). Nunca envuelto en `RetryingProvider` -- todo el sentido de un test de conexión es si el primer intento real funciona.

Construcción del adapter para la prueba (`_build_probe_adapter`, nuevo) duplica deliberadamente el switch por-proveedor que ya existe en `app/ai/provider_factory.py::_build_adapter`, en vez de reutilizarlo: `provider_factory.py` importa `app.config` (`AIProviderConfig`, `CredentialStore`), y el propio docstring de `check_provider_capability` ya se comprometía a mantener `app.ai` independiente de `app.config` (que a su vez depende de `app.ai` para `ProviderId`) -- reusar esa factory habría invertido esa dirección de dependencia. Duplicación pequeña y justificada, documentada en el propio docstring de la función.

Las dos rutas que llaman a esto (`POST /onboarding/ai-provider/validate`, `POST /providers/validate`) pasan a `async def` y encadenan: estructural primero, en vivo solo si la estructural pasó.

4 tests de `test_onboarding.py` que asumían éxito determinista contra un provider real sin mockear la red se reescriben para probar el comportamiento correcto en vez de eliminarse -- exactamente el tipo de caso que esta tarea existe para arreglar: `ollama` estructuralmente válido pero sin nada escuchando ahora falla en vivo y no avanza (antes pasaba `ok=true` falso); una credencial inventada contra Anthropic real ahora falla en vivo y nunca llega a guardarse (antes se guardaba con `ok=true` falso y el onboarding llegaba a `COMPLETE` con un provider que nunca fue alcanzable de verdad -- el bug exacto que motivó esta tarea). El test de "credencial nunca se filtra en la respuesta" pasa a usar `mock` en vez de `openai`, ya que su intención real (almacenamiento/no-filtrado) no depende de qué provider concreto se valide, y así se mantiene determinista sin mockear red.
...
### T147 — App e IA restringidas a español/inglés, con bandera de cambio en el nav
**Estado:** DONE
**Dep:** T140 (Settings, `AppConfig.language`), T146 (nav/Settings existentes a traducir)  
Petición directa del usuario (2026-09-24): tanto la app como las respuestas de la IA solo en español o inglés, español preferido/por defecto, con una bandera (española o británica) en el nav general que cambia de idioma al pulsarla -- siempre mostrando la bandera del idioma que NO está activo. Preguntado por `AskUserQuestion` si traducir todo de una vez o por fases -- el usuario eligió explícitamente "Todo ahora, en un bloque grande".

**Nota:** Backend: `SUPPORTED_LANGUAGES` (`app/config/languages.py`) se reduce de 6 códigos a exactamente `{en, es}`; `AppConfig.language` (`app/config/models.py`) cambia su default de `"en"` a `"es"`. Ningún otro sitio del backend asumía el default anterior salvo los tests, que se actualizan (`test_models.py`, `test_settings.py`). `AIOrchestrator.__init__`'s propio parámetro `language: str = "en"` se deja intacto a propósito -- es solo el valor de reserva si un caller no pasa `language` explícitamente, y en producción `app/api/dependencies.py` siempre pasa `config.language` real, así que nunca se usa fuera de tests que lo ejercitan directamente.

Frontend: sistema de traducción propio, sin librería nueva (mismo criterio que T131 con los gráficos) -- diccionario plano `Record<"namespace.key", Record<'en'|'es', string>>` (`frontend/src/i18n/translations.ts`, ~150 claves) + `LanguageContext`/`useTranslation` (React Context) envolviendo toda la app en `App.tsx`. Traducidos en este mismo bloque los ~20 componentes con texto propio: nav, dashboard, goal view, roadmap, knowledge explorer, session UI, tutor chat, reviews, assessment, project, vault diff, SQL sandbox, settings, AI provider settings, los 4 pasos de onboarding, mastery bar, voice input/read-aloud. Deliberadamente NO traducidos (documentado en el propio header de `translations.ts`): valores de enum controlados por el backend (status de goal/concept/proposal, provider ids, dominios, relation kinds) -- se muestran tal cual sea cual sea el idioma de la UI, y contenido generado por IA, que ya se resuelve por separado vía `constraints.language` (T140).

Decisión de diseño más delicada: `LanguageProvider` arranca en `'en'` de forma síncrona a propósito, aunque el nuevo default real sea `'es'` -- `GET /settings` resuelve async, y las ~93 pruebas de Vitest ya existentes renderizan y comprueban texto en inglés de inmediato sin mockear ese endpoint; arrancar en `'es'` antes de que la petición resuelva habría roto las 93 por una razón ajena a lo que cada una prueba realmente. Un usuario real ve `'es'` momentos después, en cuanto la petición real (que sí devuelve el nuevo default del backend) resuelve. Documentado en el propio docstring de `LanguageContext.tsx`.

Segunda decisión: `LanguageToggle` (nav) y el `<select>` de idioma de `SettingsView` (ya existente desde T140) escriben al mismo valor de backend -- se detectó a tiempo que serían dos estados de React independientes y se corrigió antes de terminar: `LanguageContext.setLanguage` es la única fuente de verdad (async, devuelve su propia promesa), `LanguageToggle` la llama fire-and-forget (`.catch(() => {})`, un fallo de red en un toggle de nav no merece su propio error UI), `SettingsView` la usa con `await`/`try-catch` para su propio feedback de guardado -- así ambos puntos de entrada comparten el mismo estado y nunca divergen.

Bug real encontrado al reescribir `Dashboard.tsx` en este mismo bloque: el mensaje "You have {n} review(s) due." se había traducido perdiendo el "You have"/"due." -- quedó como solo "{n} reviews", sin el resto de la frase, detectado porque `Dashboard.test.tsx` (ya existente) esperaba el texto completo. Corregido añadiendo la clave `dashboard.youHave` y reconstruyendo la frase completa en `NextAction`.

Verificado real en el navegador de esta sesión: cambiar la bandera en el nav cambia el idioma de toda la UI al instante (incluyendo el propio mensaje de confirmación "Guardado."/"Saved." al cambiarlo desde Settings, comportamiento correcto y ya probado -- `SettingsView.test.tsx` actualizado para esperar "Guardado." tras seleccionar español, no "Saved.").

18 tests nuevos de Vitest (`translations.test.ts`: cobertura completa en/es sin claves vacías ni copiadas por accidente -- 2; `LanguageContext.test.tsx`: throw fuera de provider, default inglés antes del fetch, upgrade a español tras el fetch, código no soportado ignorado, fetch fallido no rompe nada, fallback de clave ausente, `setLanguage` optimista + persistencia PATCH -- 7; `LanguageToggle.test.tsx`: bandera/etiqueta correctas en cada idioma, persistencia al pulsar -- 3) + 93 previos reescritos para envolver en `LanguageProvider` (17 archivos, `{ wrapper: LanguageProvider }` de Testing Library) + 105/105 suite frontend completa, `tsc -b` y `oxlint` limpios (un único warning nuevo, no bloqueante, `react/only-export-components` en `LanguageContext.tsx` por exportar componente + hook desde el mismo archivo, patrón ya usado en el resto del código). Backend: 1013/1013 sin cambio de conteo (tests existentes reescritos, ninguno nuevo), ruff/mypy limpios.

**Fix el mismo día:** el suite e2e de Playwright (`frontend/e2e/fresh-install.spec.ts`) no corre en el Vitest local -- solo en CI -- y seguía afirmando el copy en inglés del wizard de onboarding contra una instalación nueva, que ahora arranca en español por el nuevo default. `git push` inicial quedó en rojo en el job `e2e` (`frontend`/`backend` sí en verde) hasta un segundo commit que además encontró un bug real propio de traducción: el "in" de "Found N Markdown files **in** &lt;path&gt;" (`VaultStep.tsx`) había quedado sin traducir -- ninguna prueba de Vitest cubre ese componente, solo el e2e real lo ejercita. Corregido con una clave `onboarding.in` nueva + el spec actualizado a las cadenas en español reales. Verificado levantando un backend desechable contra una base de datos e2e limpia (`LEARNINGOS_DB_PATH` a un sqlite temporal, mismo patrón que `scripts/run_e2e_backend.py`) y recorriendo el wizard entero a mano en el navegador de esta sesión antes de hacer push otra vez -- CI quedó en verde en los tres jobs.

### T148 — Configuración por defecto de OpenRouter (modelo diario + fallback) en el wizard
**Estado:** DONE
**Dep:** T025 (rutas de onboarding), T099 (rutas de provider), T146 (Settings reutiliza las mismas rutas)  
Petición directa del usuario (2026-09-25): para OpenRouter, un modelo de uso diario sugerido (Qwen3.8 27B Free), uno de razonamiento sugerido (Nemotron 3 Ultra) y un fallback automático (`openrouter/free`, el auto-router de OpenRouter) -- visibles y editables en el wizard al elegir OpenRouter.

**Nota:** Preguntado por `AskUserQuestion` cuánto construir, dado que `AIProviderConfig` solo guardaba un modelo por proveedor y `RetryingProvider` ya tenía un parámetro `fallback` nunca usado en la práctica. El usuario eligió una mezcla explícita: el modelo de razonamiento queda solo como sugerencia de texto junto al campo "Model" (sin campo propio, sin selección automática por tipo de tarea -- no existe ningún concepto de "esta petición es de mucho razonamiento" en el dominio actual, y construirlo habría sido un cambio de arquitectura mucho mayor sin pedirlo explícitamente). El fallback sí se guarda de verdad, en un campo nuevo, y sí se conecta de verdad al parámetro `fallback` de `RetryingProvider` -- motivado explícitamente por el usuario ("dado los rate limits y otras limitaciones de peticiones API con modelos free").

IDs de modelo de OpenRouter verificados lo mejor posible: WebSearch/WebFetch contra `openrouter.ai/models` no devolvió contenido fiable (la página es un SPA client-side; tres queries de búsqueda distintas devolvieron la misma lista genérica de 20 modelos, señal de que el fetch no estaba realmente filtrando) salvo para `openrouter/free`, confirmado independientemente como el alias real del auto-router de modelos gratuitos de OpenRouter. Los IDs exactos (`qwen/qwen3.8-27b:free`, `nvidia/nemotron-3-ultra:free`, `deepseek/deepseek-r1:free`) son la mejor traducción de los nombres que dio el usuario a la convención real de slugs de OpenRouter (confirmada con `qwen/qwen3.8-max-prime`, un modelo real visto en el propio catálogo) -- el catálogo de gratuitos cambia con frecuencia y no hay forma de verificarlo con certeza total desde aquí. Riesgo mitigado, no eliminado: si un ID ya no existe, "Save & test connection" (T145, validación en vivo real) lo dirá con un error claro en el momento, igual que con cualquier modelo mal escrito a mano -- el campo queda completamente editable en el wizard, que es exactamente lo que pidió el usuario.

Backend: `AIProviderConfig.fallback_model` (nuevo, opcional) + `AppConfig.fallback_model` (staging entre los dos POST de onboarding, igual que `model`/`base_url`) + columna nueva `fallback_model` en `ai_provider_configs` (migración `a44f80553fc8`, nullable, filas existentes sin fallback). Nunca validado en vivo al guardar -- a diferencia de `model` (T145), duplicar esa llamada real por cada guardado para un modelo que puede no llegar a usarse nunca no se justificaba. `build_default_provider` (`app/ai/provider_factory.py`) construye un segundo adapter con el mismo provider/credential/base_url y solo el modelo distinto cuando `fallback_model` está presente, y lo pasa al parámetro `fallback` de `RetryingProvider` (antes siempre `None` en la práctica) -- mecanismo genérico, no específico de OpenRouter, para no meter una rama `if provider_id == "openrouter"` en el dominio (docs/AGENTS.md #20).

Frontend: `PROVIDER_OPTIONS` (`frontend/src/onboarding/providers.ts`) gana 4 campos opcionales (`defaultModel`, `reasoningModelHint`, `defaultFallbackModel`, `fallbackModelHint`), rellenados solo para `openrouter` -- el resto de proveedores no tiene un default sensato conocido (Ollama/mock dependen de lo que el usuario ya tenga localmente) y siguen arrancando en blanco como antes. `CredentialModelStep.tsx` (wizard) precarga "Model" y el nuevo campo opcional "Fallback model" con esos sugeridos y muestra las dos alternativas como texto de ayuda bajo cada campo. `AIProviderSettings.tsx` (T146, Settings post-onboarding) recibe el mismo campo -- necesario, no solo por simetría: como ambos formularios reemplazan la config completa en cada guardado (mismo comportamiento ya documentado en T146 para el resto de campos), no añadirlo ahí habría borrado silenciosamente cualquier fallback ya guardado la primera vez que alguien tocara Settings sin volver a escribirlo -- exactamente el tipo de pérdida silenciosa que prohíbe docs/AGENTS.md #18. Por eso su prellenado en Settings viene del valor YA GUARDADO (`ai_providers[].fallback_model`), nunca del sugerido del proveedor.

Verificado en el navegador de esta sesión: backend real reiniciado con las migraciones aplicadas, wizard con OpenRouter seleccionado muestra "Model" precargado con `qwen/qwen3.8-27b:free`, sugerencia de razonamiento con `nvidia/nemotron-3-ultra:free`, campo "Fallback model (optional)" precargado con `openrouter/free` y sugerencia `deepseek/deepseek-r1:free` debajo.

12 tests nuevos backend (`test_models.py`: default `None` en ambos modelos + acepta un fallback -- 3; `test_store.py`: round-trip completo incluyendo `fallback_model` -- 1; `test_provider_factory.py`: sin fallback cuando no está configurado, wiring real cuando sí -- 2; `test_onboarding.py`: se guarda junto al provider, es opcional -- 2) + 1020/1020 suite backend completa, ruff/mypy limpios, migración verificada aplicándose limpia sobre una base de datos nueva. 5 tests nuevos frontend (`CredentialModelStep.test.tsx`: prellenado + sugerencias para OpenRouter, campos en blanco para un proveedor sin defaults, el fallback se envía en el POST -- 3; `AIProviderSettings.test.tsx`: prellenado desde la config ya guardada (no desde la sugerencia), se guarda el valor actual del campo -- 2) + 110/110 suite frontend completa, `tsc -b` y `oxlint` limpios.

**Fix el mismo día:** igual que T147, el `git push` inicial quedó en rojo solo en el job `e2e` (backend/frontend en verde) -- `frontend/e2e/fresh-install.spec.ts` usa `getByLabel('Modelo')` sin `exact: true`, que hacía *substring match* por defecto contra el nuevo campo "Modelo de fallback (opcional)" (también presente para el provider `mock` que usa el e2e, ya que el campo de fallback se muestra para todos los proveedores, no solo OpenRouter) -- Playwright lo reportó como "strict mode violation: resolved to 2 elements". Corregido añadiendo `{ exact: true }` al locator. Esta vez verificado de punta a punta a mano en el navegador de esta sesión ANTES de hacer push (backend desechable + wizard completo con el provider `mock`, igual que el spec real) precisamente para no repetir el ciclo de "push a ciegas -> CI en rojo -> fix" de T147.
